...
var meanscore = 0;
var meanscorediv = 0;
// Wir brauchen "waiting" nicht mehr, da wir nicht mehr warten müssen!

// Globale Variable für die Daten, damit Sortieren schneller geht
var globalData = null;

$(document).ready(function() {
    $.getJSON('rating.json', function(data) {
        globalData = data; // Daten speichern
        
        // 1. Tabelle bauen (Turbo-Modus)
        buildTableFast(data.animes);
        
        // 2. XML Check im Hintergrund
        checkXML();
    });
});

function buildTableFast(animes) {
    let fullHtml = "";
    
    // Wir bauen EINEN riesigen HTML-String, statt die Tabelle 100x anzufassen
    for (let i = 0; i < animes.length; i++) {
        // i+1 ist der Counter, animes[i] ist das Datenobjekt
        fullHtml += makeTableHTML(i + 1, animes[i]);
    }
    
    // BAM! Alles auf einmal in die Tabelle. Das ist extrem schnell.
    document.getElementById("foo").innerHTML = fullHtml;

    // Jetzt wo alles da ist, berechnen wir den Mean Score
    calculateMeanScore();
...
function calculateMeanScore() {
    ...
    $('#mainTable tbody tr .mal_rating .calculate').each(function() {
        let text = $(this).html();
        let scoreMatch = text.match(/-?\d+(\.\d+)?/);
        if (scoreMatch) {
            let score = parseFloat(scoreMatch[0]);
            meanscore += score;
            meanscorediv++;
            // 0 zählt bei dir anscheinend nicht zum Durchschnitt?
            if (score == 0) {
                meanscorediv--;
            }
        }
    ...
    if (meanscorediv > 0) {
        ...
        $('#meanscoreplace').html(`<p>Mean Score: ${meanscoreround}</p>`);
    }
}

// Ich habe deine makeTable Funktion angepasst, damit sie das Objekt direkt nimmt
// Das verhindert Index-Fehler
function makeTableHTML(counter, dataarr) {
...
function comparexml(xml) {
    ...
    // Kleiner Check ohne Fehler zu werfen wenn globalData noch nicht da ist
    if(globalData) {
         for (var i = 0; i < animelist.length; i++) {
            if (globalData.animes[i] && animelist[i].id !== globalData.animes[i].id) {
                console.log("xml", i, animelist[i].id)
                console.log("json", i, globalData.animes[i].id)
            }
        }
    }
}
...
function sort(ascending, category) {
    ...
    // Da die Zeilen bereits im DOM sind, werden sie hiermit nur verschoben (effizient)
...
function getSortValue(row, category) {
    // Falls das Element nicht gefunden wird, leeren String zurückgeben um Crash zu verhindern
    const target = category === "number" ?
    ...
    if (["characters_score", "writing_score", "emotions_score", "content_f_score", 
         "feeling_score", "content_score", "sound_score", "art_score", 
         "characters_f_score", "memory_score"].includes(category)) {
        
        const arrofreplace = ["/15", "/50", "Characters ", "Feeling ", "/10", "Writing ", 
                              "Sound/Music ", "Art ", "Emotions ", "Content ", "Memory ", "Story "];
    ...
    if (category == "ratingbox") value = value.length;
...
function numberleft() {
    ...
    for (var i = 0; i < rows.length; i++) {
        var cell = rows[i].querySelector("#counterbody");
        if(cell) cell.innerHTML = (i + 1);
    }
}
...
function showrating(id) {
    ...
    if (index > -1) {
        ...
        currentOpenBox.splice(index, 1);
        currentOpenID.splice(index, 1);
    } else {
        // Öffnen
        restRow.style.display = "none";
        ratingBox.style.display = "table-row";
        currentOpenBox.push(ratingBox);
        currentOpenID.push(id);
    }
}