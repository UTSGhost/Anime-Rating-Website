...
$.getJSON('rating.json', function(data) {
  ...
  $('#mainTable tbody tr .mal_rating').each(function() {
    let scores = $(this).find('.calculate').html();
    if (scores) { // check if the span exists before attempting to retrieve its contents
...
function showrating(id) {
  ...
  /*for (var i = currentOpenBox.length - 1; i >= 0; i--) {
    if (currentOpenID[i] != id) {
      var openRestRow = document.getElementById(currentOpenID[i]);
      if (openRestRow) {
        openRestRow.style.display = "table-row";
      }
      currentOpenBox[i].style.display = "none";
      currentOpenBox.splice(i, 1);
      currentOpenID.splice(i, 1);
    }
  }*/
}